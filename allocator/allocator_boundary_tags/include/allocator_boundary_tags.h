...
private:
	...
	fit_result find_first_fit(size_t size);
    fit_result find_best_fit(size_t size);
    fit_result find_worst_fit(size_t size);

	void* make_allocation(const fit_result& result, size_t size);
...
private:
	...
	void merge_blocks(void* block_to_free);
};
